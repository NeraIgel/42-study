...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/01/24 19:57:10 by heha              #+#    #+#             */
/*   Updated: 2022/02/02 13:04:09 by heha             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_print_exec(t_print *print, char sign, int fd)
{
	ft_putpad_fd(' ', print->left_margin, fd);
	write(fd, &sign, print->sign);
...
static void	ft_init_sign_value(t_print *print, char *sign, t_spec *spec, int n)
{
	*sign = 0;
	if (n < 0)
		*sign = '-';
	else if (spec->flags & (1U << F_SPACE))
		*sign = ' ';
	else if (spec->flags & (1U << F_PLUS))
		*sign = '+';
	print->sign = (*sign != 0);
	print->val.i = n;
	if (n < 0)
...
static int	ft_print(t_spec *spec, va_list ap, int fd)
{
	...
	n = va_arg(ap, int);
	ft_init_sign_value(&print, &sign, spec, n);
	...
	if (spec->precision != -1)
		...
		print.zero = ft_max(spec->width - print.sign - print.val_len, 0);
	margin = ft_max(spec->width - print.sign - print.zero - print.val_len, 0);
	if (spec->flags & (1U << F_MINUS))
		print.right_margin = margin;
	else
		print.left_margin = margin;
	if ((uintmax_t)print.sign + print.zero + print.val_len + margin >= INT_MAX)
		return (-1);
	if (fd)
		ft_print_exec(&print, sign, fd);
	return (print.sign + print.zero + print.val_len + margin);
}
